...
{
    if ((n >> 7) & 1) { cpu.status |= FLAG_NEGATIVE; }
    else { cpu.status &= ~FLAG_NEGATIVE; }
    if (n == 0) { cpu.status |= FLAG_ZERO; }
    else { cpu.status &= ~FLAG_ZERO; }
}
...
void cpu_bmi() { if (cpu.status & FLAG_NEGATIVE) { cpu.pc = op_address; } }
void cpu_bcs() { if (cpu.status & FLAG_CARRY) { cpu.pc = op_address; } }
void cpu_beq() { if (cpu.status & FLAG_ZERO) { cpu.pc = op_address; } }
void cpu_bvs() { if (cpu.status & FLAG_OVERFLOW) { cpu.pc = op_address; } }

void cpu_bpl() { if (!(cpu.status & FLAG_NEGATIVE)) { cpu.pc = op_address; } }
void cpu_bcc() { if (!(cpu.status & FLAG_CARRY)) { cpu.pc = op_address; } }
void cpu_bne() { if (!(cpu.status & FLAG_ZERO)) { cpu.pc = op_address; } }
void cpu_bvc() { if (!(cpu.status & FLAG_OVERFLOW)) { cpu.pc = op_address; } }
